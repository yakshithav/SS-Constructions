...
function addPathMappingForPackage(pkgName, pathMapping) {
    const pkgDir = path.dirname(require.resolve(`${pkgName}/package.json`));
    pathMapping.set(pkgName, pkgDir);
}
async function collectTransitiveDependencies(pkgName, deps) {
...
function addSapui5TypesMappingToCompilerOptions(sapui5TypesFiles, options, context, libraryDependencies) {
...
export async function createVirtualLanguageServiceHost(options, files, sourceMaps, context, projectScriptVersion, libraryDependencies) {
    ...
    const typePathMappings = new Map();
    ...
    typePathMappings.set("@ui5/linter/types", path.dirname(require.resolve("../../../resources/types/package.json")));
    ...
    addSapui5TypesMappingToCompilerOptions(await collectSapui5TypesFiles(), compilerOptions, context, libraryDependencies);
    // Create regex matching all path mapping keys
    const pathMappingRegex = new RegExp(`^\\/types\\/(${Array.from(typePathMappings.keys()).join("|").replaceAll("/", "\\/")})\\/(.*)`);
    function mapToTypePath(fileName) {
