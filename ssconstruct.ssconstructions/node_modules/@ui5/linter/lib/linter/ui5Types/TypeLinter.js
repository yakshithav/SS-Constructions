import { createVirtualLanguageServiceHost } from "./host.js";
...
export default class TypeLinter {
    ...
    #workspace;
    #filePathsWorkspace;
    #sourceMaps = new Map(); // Maps a source path to source map content
    #sourceFileReporters = new Map();
    #libraryDependencies;
    constructor({ workspace, filePathsWorkspace, context }, libraryDependencies, sharedLanguageService) {
    ...
    async lint() {
        ...
        for (const resource of allResources) {
            ...
            if (resourcePath.endsWith(".js.map")) {
                ...
                resource.getPath().slice(0, -4), await resource.getString());
            }
            else {
        ...
        const host = await createVirtualLanguageServiceHost(this.#compilerOptions, files, this.#sourceMaps, this.#context, projectScriptVersion, this.#libraryDependencies);
        ...
        for (const sourceFile of program.getSourceFiles()) {
            ...
            const linter = new SourceFileLinter(this, sourceFile, checker, reportCoverage, messageDetails, apiExtract, this.#filePathsWorkspace, this.#workspace, ambientModuleCache, fixFactory, manifestContent, this.#libraryDependencies, this.#metadataCollector);
            await linter.lint();
            linterDone();
        }
        ...
        const virtualXMLResources = await this.#filePathsWorkspace.byGlob("/**/*.inline-*.{view,fragment}{.js,.ts,.js.map}");
        if (virtualXMLResources.length > 0) {
            for (const resource of virtualXMLResources) {
                ...
                if (resourcePath.endsWith(".js.map")) {
                    this.#sourceMaps.set(
                    // Remove ".map" from path to have it reflect the associated source path
                    resourcePath.slice(0, -4), await resource.getString());
                }
                else {
                    files.set(resourcePath, await resource.getString());
                }
            ...
            for (const sourceFile of program.getSourceFiles()) {
                ...
                const linterDone = taskStart("Type-check resource", sourceFile.fileName, true);
                const linter = new SourceFileLinter(this, sourceFile, checker, reportCoverage, messageDetails, apiExtract, this.#filePathsWorkspace, this.#workspace, ambientModuleCache, undefined, undefined, undefined, this.#metadataCollector);
                await linter.lint();
                linterDone();
            }
        }
        ...
        if (process.env.UI5LINT_WRITE_TRANSFORMED_SOURCES) {
            ...
            for (const [resourcePath, sourceMap] of this.#sourceMaps) {
                ...
                if (typeof fileContent !== "undefined") {
                    await writeTransformedSources(process.env.UI5LINT_WRITE_TRANSFORMED_SOURCES, resourcePath, fileContent, sourceMap);
                }
            }
            ...
            if (typeof byIdDts !== "undefined") {
                await writeTransformedSources(process.env.UI5LINT_WRITE_TRANSFORMED_SOURCES, CONTROLLER_BY_ID_DTS_PATH, byIdDts);
            }
        }
    }
    getContext() {
    ...
    getSourceMap(resourcePath) {
        return this.#sourceMaps.get(resourcePath);
    }
    getSourceFileReporter(sourceFile) {
        const resourcePath = sourceFile.fileName;
        let reporter = this.#sourceFileReporters.get(resourcePath);
        if (!reporter) {
            reporter = new SourceFileReporter(this.#context, sourceFile, this.#sourceMaps.get(resourcePath));
...
async function writeTransformedSources(fsBasePath, originalResourcePath, source, map) {
    const transformedWriter = createAdapter({
        fsBasePath,
        virBasePath: "/",
    });
    await transformedWriter.write(createResource({
...
//# sourceMappingURL=TypeLinter.js.map