...
export function isSourceFileOfTypeScriptLib(sourceFile) {
    return sourceFile.fileName.startsWith("/types/typescript/lib/");
}
export function isSourceFileOfPseudoModuleType(sourceFile) {
...
export function getPropertyNameText(node) {
    if (ts.isStringLiteralLike(node) || ts.isNumericLiteral(node) || ts.isIdentifier(node) ||
 ...
 */
export function getSymbolForArgumentInConstructSignatures(constructSignatures, argumentPosition) {
 ...
 * @returns symbol of the found property or undefined
 */
export function getSymbolForPropertyInConstructSignatures(constructSignatures, argumentPosition, propertyName) {
    for (const constructSignature of constructSignatures) {
 ...
 * @returns
 */
export function findClassMember(node, memberName, modifiers = []) {
 ...
 * @returns
 */
export function isClassMethod(node, checker) {
 ...
 * @returns
 */
export function getPropertyAssignmentInObjectLiteralExpression(propertyName, node) {
 ...
 * @returns
 */
export function getPropertyAssignmentsInObjectLiteralExpression(propertyNames, node) {
    const properties = [];
...
export function resolveUniqueName(inputName, existingIdentifiers) {
...
export function isReturnValueUsed(node) {
...
export function isConditionalAccess(node) {
...
export function extractNamespace(node) {
...
export function isGlobalThis(nodeType) {
    return [
...
export function getSymbolModuleDeclaration(symbol) {
 ...
 */
export function extractSapUiNamespace(symbolName, moduleDeclaration) {
    const namespace = [];
    let currentModuleDeclaration = moduleDeclaration;
    while (currentModuleDeclaration &&
 ...
 */
export function countChildNodesRecursive(node, filterKinds = []) {
    let count = 0;
...
export function findNodeRecursive(node, filterKinds = []) {
    let foundNode;
